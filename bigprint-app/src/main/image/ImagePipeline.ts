...
async function getSVGMeta(filePath: string): Promise<ImageMetaResult> {
  ...
  // Capture groups are typed `string | undefined` under noUncheckedIndexedAccess;
  // the `?? ''` fallback is safe because the outer `?` short-circuits to the
  // viewBox branch before we ever reach an empty string here.
  const w = wMatch ? parseFloat(wMatch[1] ?? '800') : vbMatch ? parseFloat(vbMatch[1] ?? '800') : 800
  const h = hMatch ? parseFloat(hMatch[1] ?? '600') : vbMatch ? parseFloat(vbMatch[2] ?? '600') : 600
...
export async function getPreviewDataUrl(
  ...
  try {
    ...
    const buffer = await sharp(filePath).resize(targetW, targetH, { fit: 'fill' }).png().toBuffer()
