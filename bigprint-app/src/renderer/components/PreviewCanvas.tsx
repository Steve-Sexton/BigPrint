...
export function PreviewCanvas() {
  ...
  const isPdfNoPreview = store.source?.mimeType === 'application/pdf' && !store.source?.previewDataUrl
  ...
  // A monotonic tick that increments whenever the container resizes. Passed
  // to usePreviewRenderer so the renderer re-runs on resize and can re-size
  // the canvas pixel buffer (otherwise the CSS stretches the old bitmap
  // producing a blurry preview until another state change triggers a redraw).
  const [resizeTick, setResizeTick] = useState(0)

  // Render
  usePreviewRenderer(canvasRef, previewImg, resizeTick)
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      if (canvasRef.current && store.source) {
        ...
        // Trigger a full redraw so usePreviewRenderer re-computes the pixel
        // buffer at the new DPR-scaled size.
        setResizeTick(n => n + 1)
      }
    })
    ro.observe(el)
  ...
  const clientToSrcCoords = useCallback(
  ...
  return (
    <div ref={containerRef} className="h-full relative overflow-hidden bg-gray-200 dark:bg-gray-900">
      {!store.source && (
