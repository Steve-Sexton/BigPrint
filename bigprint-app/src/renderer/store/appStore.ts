...
export const useAppStore = create<AppState & AppActions>()(
  immer((set) => ({
    ...
    setScale: (scale) => set(s => { Object.assign(s.scale, scale) }),
    setTiling: (tiling) => set(s => { Object.assign(s.tiling, tiling) }),
    setGrid: (grid) => set(s => { Object.assign(s.grid, grid) }),
    setInkSaver: (inkSaver) => set(s => { Object.assign(s.inkSaver, inkSaver) }),
    setZoom: (z) => set(s => { s.zoom = Math.max(0.05, Math.min(20, z)) }),
    setPan: (x, y) => set(s => { s.panX = x; s.panY = y }),
    setDarkMode: (dark) => set(s => { s.isDarkMode = dark }),
    setLoading: (loading, message = '') => set(s => { s.isLoading = loading; s.loadingMessage = message }),

    // Calibration
    setCalibrationMode: (mode) => set(s => { s.calibrationMode = mode }),
    setCalibrationPoint1: (pt) => set(s => { s.calibrationPoint1 = pt }),
    setCalibrationPoint2: (pt) => set(s => { s.calibrationPoint2 = pt }),
    setShowCalibrationDialog: (show) => set(s => { s.showCalibrationDialog = show }),
    ...
    // Crop
    setCrop: (r) => set(s => { s.crop = r }),
    setCropMode: (m) => set(s => { s.cropMode = m }),
    setCropAnchor: (pt) => set(s => { s.cropAnchor = pt }),
    setCropCurrent: (pt) => set(s => { s.cropCurrent = pt }),
    resetCropDraw: () => set(s => {
      s.cropMode = 'idle'
      s.cropAnchor = null
      s.cropCurrent = null
    ...
    // Measurement
    setMeasureMode: (m) => set(s => { s.measureMode = m }),
    setMeasurePoint1: (pt) => set(s => { s.measurePoint1 = pt }),
    setMeasurePoint2: (pt) => set(s => { s.measurePoint2 = pt }),
    resetMeasure: () => set(s => {
      s.measureMode = 'idle'
    ...
    // Ink saver preset
    setInkSaverPreset: (preset) => set(s => {
    ...
    // Page selection
    setSelectedPages: (pages) => set(s => { s.selectedPages = pages }),
    toggleSelectedPage: (row, col, totalRows, totalCols) => set(s => {
      ...
      s.selectedPages[row][col] = !s.selectedPages[row][col]
